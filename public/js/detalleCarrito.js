...
//     const nombre = document.getElementById('nombreCliente').value = usuario.Nombre || '';
//     const direccion = document.getElementById('direccion').value = usuario.Direccion || '';
...
document.getElementById('realizarPedidoDetalle')?.addEventListener('click', async () => {
  ...
  if (typeof window.realizarPedido === 'function') {
    try {
      await window.realizarPedido(datosEnvio);
      window.vaciarCarrito();
      window.guardarCarrito?.(); // Por si está expuesta también
      renderDetalleCarrito();
      mostrarAlerta('✅ Pedido realizado exitosamente');
    } catch (err) {
      console.error('❌ Error al realizar pedido:', err);
      mostrarAlerta('❌ Error al enviar el pedido');
    }
  } else {
    console.warn('⚠️ La función realizarPedido no está definida');
    mostrarAlerta('❌ No se pudo procesar el pedido. Intenta más tarde.');
  }
});
